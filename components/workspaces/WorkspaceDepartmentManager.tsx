...
import WorkspaceDepartmentDialog from "./WorkspaceDepartmentDialog";
...
export default function WorkspaceDepartmentManager({ orgId }: Props) {
  ...
  useEffect(() => { fetchDepartments(); }, [orgId]); // eslint-disable-line react-hooks/exhaustive-deps

  async function fetchDepartments() {
    setLoading(true);
    const res = await fetch(`/api/w/${orgId}/departments`);
  ...
  async function deleteDepartment(id: string) {
    ...
    await fetch(`/api/w/${orgId}/departments/${id}`, { method: "DELETE" });
    fetchDepartments();
  }
      ...
      <div>
        ...
        >
          <div className="flex items-center gap-3 flex-1 min-w-0">
            ...
            <div className="min-w-0">
              ...
              {dept.description && <p className="text-sm text-gray-500 truncate mt-0.5">{dept.description}</p>}
              {dept.headUser && <p className="text-xs text-gray-400 mt-0.5">Head: {dept.headUser.name}</p>}
            </div>
          </div>
          <div className="flex items-center gap-3 flex-shrink-0 ml-4">
            ...
            <Button variant="ghost" size="sm" onClick={() => { setEditingDept(dept); setDialogOpen(true); }} className="h-8 w-8 p-0">
              <Pencil className="h-4 w-4" />
            </Button>
            <Button variant="ghost" size="sm" onClick={() => deleteDepartment(dept.id)} className="h-8 w-8 p-0 text-red-500 hover:text-red-700 hover:bg-red-50">
        ...
        {children.length > 0 && (
          <div className="mt-2 space-y-2">
            {children.map((child) => <DeptCard key={child.id} dept={child as DepartmentWithMembers} depth={depth + 1} />)}
          </div>
        )}
      </div>
    );
      ...
      {loading ? (
        ...
        <Card className="border-dashed">
          <CardContent className="py-12 text-center">
            ...
            <Button className="mt-4" onClick={() => { setEditingDept(null); setDialogOpen(true); }}>
              <Plus className="h-4 w-4 mr-2" />
              Add Department
            </Button>
          </CardContent>
        ...
        <div className="space-y-3">
          {topLevel.map((dept) => <DeptCard key={dept.id} dept={dept as DepartmentWithMembers} />)}
        </div>
      )}

      <WorkspaceDepartmentDialog
        open={dialogOpen}
