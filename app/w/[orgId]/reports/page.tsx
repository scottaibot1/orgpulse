      ...
      {reports.length === 0 ? (
        <Card className="border-dashed">
          <CardContent className="py-16 text-center">
            ...
            <p className="text-gray-400 text-sm mt-1">Reports submitted by team members will appear here</p>
          </CardContent>
        </Card>
      ) : (
                    ...
                    {report.parsedReport.tasks.map((task) => (
                      <div key={task.id} className="flex items-start justify-between gap-3">
                        <div className="flex items-start gap-2 min-w-0">
                          <span className="text-xs text-gray-400 mt-0.5 flex-shrink-0">#{task.priorityRank}</span>
                          <div className="min-w-0">
                            <p className="text-sm text-gray-800 truncate">{task.description}</p>
                            {task.projectName && <p className="text-xs text-gray-400">{task.projectName}</p>}
                          </div>
                        </div>
