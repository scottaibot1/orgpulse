...
export default async function PersonDetailPage({ params }: Props) {
    ...
    include: {
      ...
      reports: {
        include: {
          parsedReport: {
            include: {
              tasks: { orderBy: { priorityRank: "asc" } },
            },
          },
        },
  ...
  const totalTasks = person.reports.reduce(
    (sum, r) => sum + (r.parsedReport?.tasks.length ?? 0),
    0
  );
  const blockedTasks = person.reports
    .flatMap((r) => r.parsedReport?.tasks ?? [])
    .filter((t) => t.status === "blocked").length;

  const submissionLink = `${process.env.NEXT_PUBLIC_APP_URL}/submit/${person.submissionToken}`;
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-start gap-4">
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            {submittedToday ? (
              <Badge className="bg-green-100 text-green-700">
                <CheckCircle className="h-3 w-3 mr-1" />Submitted today
              </Badge>
            ) : (
              <Badge className="bg-red-100 text-red-700">
                <Clock className="h-3 w-3 mr-1" />Not submitted today
              </Badge>
            )}
          </div>
          ...
          <div className="flex flex-wrap items-center gap-3 mt-2">
            ...
            {person.departmentMemberships.map((m) => (
              <Badge
                key={m.id}
                variant="outline"
                style={m.department.color ? {
                  borderColor: m.department.color,
        ...
        <div className="flex-shrink-0">
          <CopyLinkButton link={submissionLink} />
        </div>
      </div>
            ...
            <Card key={report.id} className="overflow-hidden">
              ...
              {report.parsedReport && (
                <CardContent className="p-4 space-y-3">
                      ...
                      {report.parsedReport.tasks.map((task) => (
                        ...
                        >
                          ...
                          <div className="flex items-center gap-2 flex-shrink-0">
                            {task.hoursToday !== null && (
                              <span className="text-xs text-gray-400">{String(task.hoursToday)}h</span>
                            )}
                            {task.pctComplete !== null && (
                              <span className="text-xs text-gray-500 font-medium">{task.pctComplete}%</span>
                            )}
                  ...
                  {report.parsedReport.totalHours !== null && (
                    <p className="text-xs text-gray-400">
                      Total hours: <span className="font-medium">{String(report.parsedReport.totalHours)}h</span>
                    </p>
                  )}
                </CardContent>
              )}
