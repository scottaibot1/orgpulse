...
REASONING ABOUT PRIORITY AND STATUS: After understanding every record, apply intelligent prioritization. A task whose due date has already passed and is not marked complete is a problem — surface it prominently with its original due date. A task with a near future due date and low completion percentage is a risk. A task explicitly marked complete or at 100% belongs in completed regardless of what section of the document it appears in. Anything the person says they plan to do tomorrow or next is their stated next actions. Never leave a task in a vague unclassified state when the document contains enough context to classify it — ask yourself what a smart analyst would conclude about this task's status, urgency, and priority from everything on the page.
